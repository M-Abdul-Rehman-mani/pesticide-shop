...
class DashboardScreen(QWidget):
    ...
    def _display(self, result: object) -> None:
        ...
        self.cards["Returns"].set_value(format_money(metrics.returns, self._currency))
        self.cards["Damage"].set_value(format_money(metrics.damage, self._currency))
        self.cards["Profit"].set_value(format_money(metrics.profit, self._currency))
        self.cards["Phones Sold"].set_value(str(metrics.phones_sold))
        self.cards["Phones Returned"].set_value(str(metrics.phones_returned))
        self.cards["Damaged Phones"].set_value(str(metrics.damaged_phones))
        self.cards["Current Inventory"].set_value(str(metrics.current_inventory))
        self.cards["Low Stock Models"].set_value(str(metrics.low_stock_models))
        ...
        sales_axes.set_title("Sales and returns by day")
        if chart_rows:
            ...
            sales_axes.plot(
                x_values,
                [float(row.returns) for row in chart_rows],
                marker="o",
                color="#eb5757",
                label="Returns",
            )
            ...
            profit_axes.plot(
                x_values,
                [float(row.damage) for row in chart_rows],
                marker="o",
                color="#f2994a",
                label="Damage loss",
            )
            ...
            for axes, message in (
                (sales_axes, "No sales or returns"),
                (profit_axes, "No profit or damage data"),
            ):
                axes.text(0.5, 0.5, message, ha="center", va="center")
                axes.set_axis_off()
        profit_axes.set_title("Profit and damage")
        top_axes.set_title("Top-selling phones")
        if top_models:
